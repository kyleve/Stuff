...
`store.perform { ... }` or `store.performInCurrentGeneration { ... }`. Attribution revisions use this same boundary; their
coordinator also requires the reviewed data generation and reassesses before writing.
