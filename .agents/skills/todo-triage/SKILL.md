...
6. **Push the branch and open a PR** ready-for-review. Follow the
   [`github-workflow`](../github-workflow/SKILL.md) skill. Describe the end
...
1. **Understand what's being claimed.** An entry like "Raw data browser (similar
   ...
   logout?" is a question that can or cannot hide a bug. Resolve which before
   going further.
2. **Verify it against current source.** Find the code. Make sure that the behavior is
...
5. **Bucket and tag it.** `PX`/`P0`/`P1`/`P2`, plus `quick-win` or
   ...
   Priority is a decision someone made. A severity opinion from a later pass
