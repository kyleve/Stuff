...
6. Enables and runs `BackupServiceTests.upgradedBackupDecodesAndLoadsAssets`
   through `./test`, exercising `BackupService.readArchive(at:)` and all
   referenced asset bytes. The test is compiled but disabled during normal runs.
