...
struct CaptureStoreTests {
    ...
    func stagesOriginalResourcesAndRemovesOnlyStagedCopies(hasRAW: Bool) async throws {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString)
        defer { do { try FileManager.default.removeItem(at: root) } catch { Issue.record(error) } }
        let store = try CaptureStore(root: root)
        let imageID = CaptureSequence.Slot.ID(rawValue: UUID())
        let capture = CameraCapture(jpeg: Data("jpeg".utf8), raw: hasRAW ? Data("raw".utf8) : nil)
        try await store.stageCapture(capture, imageID: imageID)
        let jpegURL = await store.imageURL(imageID, resource: .original)
        #expect(try Data(contentsOf: jpegURL) == capture.jpeg)
        let rawURL = await store.rawURL(imageID)
        #expect((rawURL != nil) == hasRAW)
        if let rawURL { #expect(try Data(contentsOf: rawURL) == capture.raw) }
        try await store.removeStagedFiles(imageID: imageID)
        #expect(!FileManager.default.fileExists(atPath: jpegURL.path))
        #expect(await store.rawURL(imageID) == nil)
    }
}
