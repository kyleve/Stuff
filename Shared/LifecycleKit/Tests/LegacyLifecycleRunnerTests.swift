...
@MainActor
struct LegacyLifecycleRunnerDriveTests {
    @Test func runsStepsInDeclarationOrder() async {
        ...
        let runner = LegacyLifecycleRunner(reason: .userForeground, sequence: LegacyLifecycleSteps {
            LegacyLifecycleStep.work("a") { _ in executed.append("a") }
            LegacyLifecycleStep.work("b") { _ in executed.append("b") }
            LegacyLifecycleStep.work("c") { _ in executed.append("c") }
        })
        await runner.run()
    ...
    @Test func skipsStepsWhoseConditionIsFalse() async {
        ...
        let runner = LegacyLifecycleRunner(reason: .userForeground, sequence: LegacyLifecycleSteps {
            LegacyLifecycleStep.work("a") { _ in executed.append("a") }
            LegacyLifecycleStep.work("skip", condition: { false }) { _ in executed.append("skip") }
            LegacyLifecycleStep.work("c") { _ in executed.append("c") }
        })
        await runner.run()
        #expect(executed == ["a", "c"])
    ...
    @Test func filtersStepsByLaunchReason() async {
        ...
        let runner = LegacyLifecycleRunner(
            ...
            sequence: LegacyLifecycleSteps {
                LegacyLifecycleStep.work("always") { _ in executed.append("always") }
                LegacyLifecycleStep.work("fg", modes: .foreground) { _ in executed.append("fg") }
                LegacyLifecycleStep.work("bg", modes: .background) { _ in executed.append("bg") }
            },
        )
        await runner.run()
    ...
    @Test func interactiveStepsAreSkippedInBackground() async {
        ...
        let runner = LegacyLifecycleRunner(
            ...
            sequence: LegacyLifecycleSteps {
                LegacyLifecycleStep.work("a") { _ in executed.append("a") }
                LegacyLifecycleStep
                    .interactive("onboarding", perform: { _ in
    ...
    @Test func runIsIdempotent() async {
        var count = 0
        let runner = LegacyLifecycleRunner(reason: .userForeground, sequence: LegacyLifecycleSteps {
            LegacyLifecycleStep.work("a") { _ in count += 1 }
        })
        await runner.run()
        await runner.run()
        #expect(count == 1)
...
@MainActor
struct LegacyLifecycleRunnerForegroundPromotionTests {
    @Test func enterForegroundReDrivesAndRunsForegroundOnlySteps() async {
        var executed: [String] = []
        let runner = LegacyLifecycleRunner(
            reason: .background(.location),
            sequence: LegacyLifecycleSteps {
                LegacyLifecycleStep.work("store") { _ in executed.append("store") }
                LegacyLifecycleStep
    ...
    @Test func retryAfterPromotionSkipsAStepAlreadyCompletedInTheHeadlessDrive() async throws {
        ...
        let runner = LegacyLifecycleRunner(reason: .undetermined, sequence: LegacyLifecycleSteps {
            LegacyLifecycleStep.work("store") { _ in executed.append("store") }
            LegacyLifecycleStep.work("onboarding", modes: .foreground) { _ in
                executed.append("onboarding")
                if onboardingShouldFail { throw StepError() }
            }
            LegacyLifecycleStep.work("widget") { _ in executed.append("widget") }
        })
    ...
    @Test func backgroundWorkStepCallingWaitForResolutionDoesNotReachReadyUntilPromoted(
        ...
        let runner = LegacyLifecycleRunner(
            reason: .background(.location),
            sequence: LegacyLifecycleSteps {
                LegacyLifecycleStep.work("wait") { bridge in try await bridge.waitForResolution() }
            },
        )
    ...
    @Test func enterForegroundCancelsAndDrainsAnInFlightBackgroundDrive() async throws {
        ...
        let runner = LegacyLifecycleRunner(
            reason: .background(.location),
            sequence: LegacyLifecycleSteps {
                LegacyLifecycleStep.work("slow") { bridge in
                    starts += 1
                    inFlight += 1
                    defer { inFlight -= 1 }
...
@MainActor
struct LegacyLifecycleRunnerCancellationTests {
    @Test func resetCancelsAParkedInteractiveStepInsteadOfHanging() async throws {
        ...
        let runner = LegacyLifecycleRunner(reason: .userForeground, sequence: LegacyLifecycleSteps {
            ...
            // completion.
            LegacyLifecycleStep.interactive("gate", condition: { !teardownRan }) { _ in
                Text("gate")
            }
        })
        let runTask = Task { @MainActor in await runner.run() }
        ...
        await runner.teardown(LegacyLifecycleSteps {
            LegacyLifecycleStep.work("teardown") { _ in teardownRan = true }
        })
...
@MainActor
struct LegacyLifecycleRunnerFailureTests {
    @Test func thrownErrorParksInFailedAndStopsSubsequentSteps() async {
        var executed: [String] = []
        let runner = LegacyLifecycleRunner(reason: .userForeground, sequence: LegacyLifecycleSteps {
            LegacyLifecycleStep.work("a") { _ in executed.append("a") }
            LegacyLifecycleStep.work("b") { _ in throw StepError() }
            LegacyLifecycleStep.work("c") { _ in executed.append("c") }
        })
        await runner.run()
        #expect(executed == ["a"])
    ...
    @Test func retryIsNoOpWhenFailedStepIDDoesNotMatchAnyStep() async {
        var executed = 0
        let runner = LegacyLifecycleRunner(reason: .userForeground, sequence: LegacyLifecycleSteps {
            LegacyLifecycleStep.work("a") { _ in executed += 1 }
        })
        await runner.run()
        #expect(runner.phase.isReady)
    ...
    @Test func bridgeFailurePropagatesToFailedPhase() async throws {
        let runner = LegacyLifecycleRunner(reason: .userForeground, sequence: LegacyLifecycleSteps {
            LegacyLifecycleStep.interactive("gate") { _ in Text("x") }
        })
...
@MainActor
struct LegacyLifecycleRunnerInteractiveTests {
    @Test func interactiveStepSuspendsUntilResolved() async throws {
        var executed: [String] = []
        let runner = LegacyLifecycleRunner(reason: .userForeground, sequence: LegacyLifecycleSteps {
            LegacyLifecycleStep.work("a") { _ in executed.append("a") }
            LegacyLifecycleStep.interactive("gate") { _ in Text("gate") }
            LegacyLifecycleStep.work("c") { _ in executed.append("c") }
        })
        let task = Task { @MainActor in await runner.run() }
        try await waitUntil { runner.phase.isRunning("gate") }
    ...
    @Test func alwaysPresentationActivatesImmediately() async throws {
        let runner = LegacyLifecycleRunner(reason: .userForeground, sequence: LegacyLifecycleSteps {
            LegacyLifecycleStep.interactive("ui") { _ in Text("ui") }
        })
        let task = Task { @MainActor in await runner.run() }
    ...
    @Test func whenFalsePresentationStaysSilent() async throws {
        let runner = LegacyLifecycleRunner(reason: .userForeground, sequence: LegacyLifecycleSteps {
            LegacyLifecycleStep.work("s") { bridge in try await bridge.waitForResolution() }
    ...
    @Test func deferredPresentationActivatesAfterDelay() async throws {
        let runner = LegacyLifecycleRunner(reason: .userForeground, sequence: LegacyLifecycleSteps {
            LegacyLifecycleStep.work("slow") { bridge in try await bridge.waitForResolution() }
    ...
    @Test func minVisibleHoldsAnAlwaysPresentationAfterAFastStep() async {
        ...
        let runner = LegacyLifecycleRunner(reason: .userForeground, sequence: LegacyLifecycleSteps {
            LegacyLifecycleStep.work("fast") { _ in }
    ...
    @Test func progressIsReadableWhileStepRuns() async throws {
        let runner = LegacyLifecycleRunner(reason: .userForeground, sequence: LegacyLifecycleSteps {
            LegacyLifecycleStep.interactive("p", perform: { bridge in
                bridge.progress = 0.5
                try await bridge.waitForResolution()
            }) { _ in Text("p") }
        })
