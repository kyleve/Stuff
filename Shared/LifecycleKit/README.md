...
hole downstream, the app UI rendering off an optional that was expected to have been
...
- **`thenKeeping` steps** are required `Void`-output work. The trunk valueflows past them, so they *may* gate on the launch reason.
...
Model each step as a type. Assemble the plan. Build the runner early (for example in.
...
nodes, then relaunches from the top as a fresh attempt — for example a logout/erase
...
synchronously at `init` for cheap, must-exist-now wiring (for example installing a
