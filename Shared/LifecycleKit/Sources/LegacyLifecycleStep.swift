...
/// drive UI directly.
public struct LegacyLifecycleStep: Identifiable {
...
extension LegacyLifecycleStep {
    ...
    public static func work(
        ...
        LegacyLifecycleStep(id: id, modes: modes, condition: condition, perform: perform)
    }
...
struct LifecycleStepPresentation {
    ...
    /// heterogeneous `[LegacyLifecycleStep]` can share one element type without leaking
    /// each step's concrete view type into `LegacyLifecycleStep`/`LegacyLifecycleSteps`; the
