    ...
    enum InspectorPreviewData {
        @MainActor
        static func populatedConfiguration() -> InspectorSwiftDataConfiguration {
        ...
        @MainActor
        static func emptyConfiguration() -> InspectorSwiftDataConfiguration {
            InspectorSwiftDataConfiguration(
        ...
        static func tablePreview() -> some View {
            TablePreviewHost(
                model: InspectorSwiftDataModel(configuration: populatedConfiguration()),
            )
        }
    ...
    private struct RowDetailPreviewHost: View {
        ...
        var body: some View {
            ...
            .task {
                ...
                entity = book
                do {
                    row = try await model.rows(for: book).rows.first
                } catch {
                    model.report(error)
                }
            }
    ...
    private struct RelationshipPreviewHost: View {
        ...
        var body: some View {
            ...
            .task {
                ...
                entity = author
                do {
                    rowID = try await model.rows(for: author).rows.first?.persistentID
                } catch {
                    model.report(error)
                }
            }
