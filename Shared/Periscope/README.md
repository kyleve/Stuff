...
Durability underneath the store comes from [`JournalKit`](../JournalKit), the generic append-only crash-durable journal.
It is deliberately payload-agnostic, so it knows nothing about log semantics.

[`Prototypes/JournalBenchmark`](Prototypes/JournalBenchmark) is a standalone macOS benchmark harness that informed the journal design.
...
Cross-module design and follow-up work for the whole stack is tracked in one place: [`TODOs.md`](TODOs.md).
