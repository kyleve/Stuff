...
- fix: `PeriscopeViewer` nests a `TabView` inside the host `NavigationStack`, and both tabs push onto that same stack (Logs → detail / `SpanTreeView`, Hierarchy → `ScopeEventsView`). A detail pushed from Logs lingers when you switch to Hierarchy, and per-tab title/toolbar/`searchable` can bleed across tabs. Give each tab its own `NavigationStack` (or have the viewer own the stack), and add a hosting test that pushes a detail, switches tabs, and asserts the back stack. (Log-viewer PR #107 review.)
- fix: Hierarchy subtree counts tally by `primaryScope` only (`LogHierarchyModel.directCounts(in:)`), but the drill-in (`LogInspectorModel` with `LogQuery.scope = .subtree`) matches events linking *any* scope in the subtree — so a scope's badge count and its drilled-in list length can disagree (compounded by the count being uncapped while the drill-in caps at `limit: 500`). Count by "any linked scope in the subtree" to match the query, or document the primary-only semantics. (Log-viewer PR #107 review.)


## P2s (Nice to have)
- fix: `SpanTreeModel.buildTree` derives `duration` via `try? decode(SpanEnded)` while `exitMode` reads the stored `spanExitMode` column, so a payload decode failure renders an exit chip *and* a "running" duration for an ended span; the span-name fallback also strips a magic `"▶ "` prefix off the message. Log the decode failure and drive duration/open-state from one source so an ended span can't read as running. (Log-viewer PR #107 review — "never silently swallow errors".)
- design: `SpanTreeModel` models open spans with `effectiveEnd = .distantFuture`, so every span that begins later — even an independent, concurrent one — nests under any still-open span, collapsing the later tree into one deep chain. Confirm the intended containment semantics for open spans and pin them with a test, or nest more conservatively. (Log-viewer PR #107 review.)
- fix: `SpanTreeView` seeds `\.logRowDensity` but `SpanTreeRow` hard-codes `row.comfortable` and never reads it — a no-op that touches `UserDefaults` on every render. Drop the modifier, or make span rows density-aware. (Log-viewer PR #107 review.)
- refactor: `ScopeEventsView`, `LogInspectorView`, and `SpanTreeView` seed density from `.load(from: .standard)` directly, bypassing the injectable `defaults` the viewer threads through — so those surfaces can't be pointed at an ephemeral test suite and always touch the shared standard domain. Thread `defaults` through (or read the density the viewer already seeds into the environment). (Log-viewer PR #107 review.)
- perf: The incremental *fetch* is bounded per commit, but `SpanTreeModel.load`/`LogHierarchyModel.load` still rebuild the whole tree/forest from all accumulated events on every `changes()` ping — O(total spans) per commit for a long-lived viewer over a busy store. Rebuild incrementally or throttle rebuilds. (Log-viewer PR #107 review.)
- fix: `LogHierarchyModel`/`SpanTreeModel` `.failed` states set honest UI state (rendered via `ContentUnavailableView`) but don't log the error (matches `LogInspectorModel`'s style); the root convention prefers surfacing state *and* logging. Add a log on the `catch`. (Log-viewer PR #107 review.)
