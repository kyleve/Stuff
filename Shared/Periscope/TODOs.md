...
- perf: `Periscope.chunked(_:)` groups pending items with `array + [element]`, which is quadratic in batch size — a full 5,000-record backlog does millions of element copies inside the drain. Append into a mutable current-chunk buffer instead. (Code review finding 7.)
