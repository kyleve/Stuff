...
visible main-UI reveal. An app root should supply active-scene visibility so an
...
must be), forwards `enterForeground()` / `teardown(_:input:)`,
...
terminal `ContentUnavailableView`, no retry button) are used by the convenience
