...
/// views to reach `enterForeground()`/`teardown(input:_:)`. A failed launch
...
>: View {
    ...
    /// the failure surface — visible and named. Identical in debug and
    /// release, which also keeps the path testable.
    ///
