...
- [`LogViewerModel`](Sources/LogViewerModel.swift) – the `@MainActor
  ...
  `filteredEntries`, owns the filter state (`searchText`, `minimumLevel`,
  `selectedCategory`), and renders `exportText`. Keep store work out of here
  beyond `snapshot()`/`clear()`/`observe()`.
...
- **`observe()` is bound to the view's lifetime.** It's launched from
  `LogViewer`'s `.task` and ends when the view goes away (the stream's consumer
  cancels, which unregisters the observer in `LogStore`). Don't detach it into a
  long-lived `Task`.
...
- **Newest-first for display, oldest-first for export.** `filteredEntries`
  ...
  shared/copied log reads chronologically. Keep both directions.
