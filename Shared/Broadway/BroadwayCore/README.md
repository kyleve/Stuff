...
`BContext` holds a `BStylesheets` cache.
The cache rebuilds whenever `baseTraits`, `traitOverrides`, or `themes` change (via `didSet`).
The cache is `@EquatableIgnored`.
Two contexts compare equal on their inputs, not on the derived cache.
`BContext+UITraits.swift` bridges the context onto a
`UITraitCollection` custom trait (guarded by `#if canImport(UIKit)`).

## Install

`BroadwayCore` is a local SPM library declared in the root [`Package.swift`](../../../Package.swift).
Add it with `.package(product: "BroadwayCore")`.
