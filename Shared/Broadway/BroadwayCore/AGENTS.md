...
- **It writes newly-created sheets into the copy-on-write box in place through `_unsafeUnderlyingValue`.**
- **Value copies of a context share one cache.** A stylesheet is created once per `(type, traits, themes)` key.
- **A trait or theme change only moves the key.** Entries under the old key stay in the dictionary.
- **Nothing evicts them today.** See the `TODO` in `BStylesheets.swift`.
- **Lookups resolve fresh sheets under the new key.** Do not assume reading `context.stylesheets` re-resolves.
