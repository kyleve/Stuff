...
UIKit hosting and run-loop helpers for the hosted Swift Testing bundles that run inside [`StuffTestHost`](../StuffTestHost).
It is the single, dependency-free home for the helpers that used to be duplicated across `WhereTesting` and `BroadwayTesting`.

## Install

Library product in the root [`Package.swift`](../../Package.swift) (`Shared/TestHostSupport/Sources`).
It is **test-only**.
Hosted test bundles depend on it (via the `unitTests` helper in [`Project.swift`](../../Project.swift)).
The `StuffTestHost` app also links it so its `SceneDelegate` can mark the host window.
...
`StuffTestHost`'s `SceneDelegate` creates one window, gives it a root view controller, and sets `window.isMainTestHostWindow = true`.
...
`show()` waits (pumping the run loop) for the host window and its root view controller to exist before hosting.
A test that runs before the host scene has connected does not spuriously fail.

## Contracts & limitations

- **The marker is an associated object keyed on a name-interned `Selector`.** This module is a static library embedded into every image that links it (the host app and each `.xctest` bundle).
