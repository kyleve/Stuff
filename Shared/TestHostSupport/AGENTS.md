...
- **`show` waits for readiness.** It pumps the run loop for the host window and root VC before hosting. Then a test that runs before the scene connects does not fail spuriously. It follows Apple's parent/child VC order. It always restores `layer.speed` through a `defer` at entry.
