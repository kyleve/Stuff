...
struct SwiftDataInspectorTests {
    ...
    @Test func binaryDescriptionReportsInlineBytesOrPlaceholder() {
        #expect(SwiftDataInspectorReader.binaryDescription(Data(count: 12)) == "12 bytes")
        #expect(SwiftDataInspectorReader.binaryDescription("not data") == "Data")
        #expect(SwiftDataInspectorReader.binaryDescription(Data?.none as Any) == "Data")
    }
    ...
    @Test func defaultFormatUsesNumericDateStyle() throws {
        ...
        let formatted = SwiftDataInspectorReader.defaultFormat(date)
    ...
    @Test func skipsCountWhenFetchProvesNoTruncation() async throws {
        ...
        let model = SwiftDataInspectorModel(
            configuration: SwiftDataInspectorConfiguration(container: container, rowLimit: 10),
        )
        await model.loadEntities()
        let widget = try #require(model.entities.first { $0.name == "TestWidget" })
        let rowSet = await model.rows(for: widget)
