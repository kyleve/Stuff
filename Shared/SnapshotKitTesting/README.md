...
(zero by default. A frame's `safeAreaInsets`, for example `.iPhoneNotched`,.
...
- Runs in a hosted test bundle (needs a host app window. In this repo that's`StuffTestHost`, reached via `TestHostSupport`).
