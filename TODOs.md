...
## P0s (Must do)
- fix(Bumper) [quick-win]: `where.gregorian_calendar` matches only an explicit `Calendar` base, so it enforces nothing. It filters `MemberAccessExprSyntax` on `base?.trimmedDescription == "Calendar"` (`.bumper/Sources/WhereProjectRules.swift:124-125`, rule at `:117-137`, `severity: .error` at `:119`), which catches a spelled-out `Calendar.current` but not the implicit-member form (`calendar: Calendar = .current`, `startOfDay(in: .current)`) — and after the Gregorian call-site pass (`fe99dde`) the implicit form is the only one left in the tree: **still 12 sites** (re-counted 2026-08-16), four of them shipped production paths and eight in DEBUG snapshot/preview fixtures (enumerated in the `CalendarDay.displayDate` P1 in [`Where/TODOs.md`](Where/TODOs.md)). CI still hard-gates the lint and is green, which confirms the rule reports none of them — though **the gate moved this week**: PR #271 replaced the three `swift run bumper` steps with `./test --architecture-only`, so the citation is now the `architecture` job at `.github/workflows/ci.yml:62-63`, which reaches `bumper config`/`test`/`lint` through `test:186-202`. **Why it has survived four audits:** the rule's own mutation test only ever feeds it a spelled-out `Calendar.current` (`.bumper/Tests/WhereProjectRulesTests.swift:167-177`), so the test passes for the same reason the rule fails — fix both together, and add an implicit-member case to the test first. Also match a no-base `MemberAccessExprSyntax` whose contextual type is `Calendar`, or add a lexical `.current` check scoped to calendar parameters and arguments. A rule that reads as enforced but enforces nothing is worse than a documented convention, because it stops anyone from looking. (audit 2026-07-26; re-verified 2026-08-09)

## P1s (Should do)
- docs(CI) [quick-win]: Finish reconciling the repo's description of CI with the two-system split PR #237 created. **iOS unit tests and snapshots now run on CircleCI** (`test-ios` and `snapshot`, `m4pro.medium`, Xcode 27.0 — `.circleci/config.yml:199-236`), while GitHub Actions keeps `format`, `architecture`, and `test-macos` (`.github/workflows/ci.yml:22-107`); CircleCI passes `--skip-architecture` so Bumper is not run twice. Two halves are already corrected in this pass (root [`AGENTS.md`](AGENTS.md)'s "Full build & test" section, which claimed CI *is* `.github/workflows/ci.yml`, and that workflow's own `test-macos` comment, which still said it ran "in parallel with the iOS `test` job" that no longer exists there). What remains:
	- **Restore the warning that PR #237 dropped.** The old GitHub Actions snapshot job carried a paragraph explaining why the suite must not be parallelized — `-parallel-testing-enabled` distributes XCTest *classes* and Swift Testing presents none, so everything lands on one worker and Swift Testing's own in-process parallelism interleaves captures inside a single `StuffTestHost` and corrupts them (`41bdb0e5:.github/workflows/ci.yml:147-154`). That comment did not travel to `.circleci/config.yml`, and nothing in either CI file says it now, so the next person tuning a slow snapshot job meets no warning at the place they would edit. The reasoning itself survives in [`SnapshotKitTesting/AGENTS.md`](Shared/SnapshotKitTesting/AGENTS.md) — put a short version back beside the CircleCI job and point at it.
	- **Reconcile the "what CI runs" claims.** `./test --everything` is described as "what CI runs" ([`test`](test):98, `:133`) and the [`running-tests`](.agents/skills/running-tests/SKILL.md) skill heads its recipe "Full macOS validation (matches CI)" (`SKILL.md:120`), but neither covers the `format` job's `./sf-symbols --lint`, `./xcstrings --lint`, and `./attribution --check`, nor the `architecture` job's `./test --architecture-only`. Either widen the recipe or stop calling it a match. (audit 2026-08-16)
...
- feat(Scripts) [needs-design]: Teach [`test`](test) the native-macOS tier, or stop stating that it is the only entry point. The root [`AGENTS.md`](AGENTS.md#running-tests) says "**Use `./test`** — the only way to run tests. Never hand-roll `tuist test` or `xcodebuild`", but the Swift [`TestRunPlan`](Tools/Sources/StuffToolCore/TestRunPlan.swift) / [`TestService`](Tools/Sources/StuffToolCore/TestService.swift) has no Ledger scheme or macOS destination, so `LedgerCoreTests` — a real bundle with its own CI job — simply cannot be run through it. The [`running-tests`](.agents/skills/running-tests/SKILL.md) skill already documents the exception with the raw command (`tuist test Ledger-macOS-Tests -- -destination 'platform=macOS'`), and `LedgerCore/AGENTS.md` gives its own variant, so the truth lives in two places while the always-applied root rule contradicts both. An agent that reads only the root file — which is the one guaranteed to be loaded — concludes Ledger's tests go through `./test`, and nothing tells it otherwise until the command fails. Either add a macOS tier to `./test` (it already resolves destinations through [`simulator`](simulator) for iOS, and a macOS run needs no device at all, so this is the smaller change than it looks) or make the root rule name the carve-out explicitly. **The doc half is done** — root `AGENTS.md` now points at the skill for the macOS bundle — so what remains is deciding whether the command should absorb the tier. (audit 2026-08-09; paths refreshed 2026-08-13)
- refactor(Scripts) [needs-design]: The root dev scripts want an overhaul — there are **17** now (re-counted 2026-08-16, including `snapshot-shards` and `shellcheck`), and three of them duplicate the same xcodebuild plumbing. `./test`, [`profile`](profile), and [`flaky`](flaky) genuinely want different run semantics, but share destination resolution, typed xcresult traversal, and output conventions. Revisit that boundary as part of the planned shell/Ruby tooling simplification rather than folding every policy into `./test`. (human 2026-07-28; paths refreshed 2026-08-16)
- refactor(Scripts) [needs-design]: Evaluate `tuist xcodebuild test-without-building` as a way to retire `./test`'s affected-bundle parser. The Swift migration replaced the regex parser with Tuist's JSON graph, but the same local-convenience question remains for the planned shell/Ruby simplification. Verify raw diagnostic preservation and behavior with an empty Tuist hash cache before adopting it. (agent 2026-07-28; paths refreshed 2026-08-16)
...
- refactor(Scripts) [needs-design]: The affected-bundle selector regex-parsed
  ...
  Closed by moving `./test` into `StuffToolCore`: it now reads Tuist's pinned
  `graph --format json` output for schemes, targets, sources, and hosts, then
  joins that to `swift package dump-package` for the local-package closure.
  Raw `xcodebuild` remains deliberate because it preserves Swift Testing's
  diagnostic detail and supports `-collect-test-diagnostics never`; minimized
  graph/package fixtures and a live `./test StuffCoreTests` run guard the new
  boundary. (agent 2026-07-28, closed 2026-08-09)
