...
- refactor(Scripts) [needs-design]: Finish the bounded Swift tooling migration.
  The high-risk Xcode cluster (`./simulator`, `./test`, `./profile`, and
  `./flaky`) now dispatches into the independent `Tools` package with typed,
  directly tested orchestration and separate command semantics; `./icons` now
  uses a validated, rollback-safe file transaction, and `./Where/install` uses
  typed `devicectl` selection. `./Ledger/install` now uses exact process matching
  and staged replacement. What remains is extracting the last substantive
  Python embedded in retained shell and focused regression coverage for the
  retained Ruby tools; close this item when those hardening checks pass. (human
  2026-07-28; migration in progress 2026-08-10)
