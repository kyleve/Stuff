...
- refactor(Scripts) [needs-design]: Finish the bounded Swift migration of the
  Xcode orchestration cluster. `./test` and `./profile` now dispatch into the
  independent `Tools` package and share typed simulator, subprocess, raw-log,
  and xcresult boundaries while retaining separate command semantics. `./flaky`
  remains the last large shell implementation in this cluster, including its
  suite/tight-loop xcresult aggregation and report generation; port it over the
  shared boundaries, preserve its report-only exit policy and warm DerivedData,
  then close this item. (human 2026-07-28; migration in progress 2026-08-10)
