...
- refactor(Scripts) [needs-design]: Complete the approved four-PR hardening pass over the **18** root commands. PR 1 establishes pinned ShellCheck, direct Python/Ruby tests, and an importable TLA runner. PR 2 extracts the substantive Python from `test`, `profile`, and `flaky` behind thin shell orchestration while preserving their deliberately different run semantics. PR 3 hardens simulator ownership, locking, and exact-runtime deletion. PR 4 makes icon and installer mutations transactional and dry-runnable. Keep public paths, flags, output, and exit codes stable; retain shell for bootstrap/process orchestration and prefer Ruby for filesystem-heavy tools. Review and merge each PR before starting the next so every phase is independently reversible. (human 2026-07-28; plan approved 2026-08-17)
