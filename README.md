...
To see where build and test time goes, run `./profile` — it prints setup/build/test walls, the slowest build phases and tests, slow type-check sites, and per-phase snapshot capture costs. It also writes a versioned per-suite snapshot report that `./snapshot-shards balance --report PATH` can consume. The default reuses unit-build products for the snapshot build; `--ci-shape` instead gives each scheme cold DerivedData like its independent CI job. It only reports, it never fails on slow numbers; see `./profile --help` for the remaining scope, destination, and threshold flags.

CI runs snapshots as two serial suites on separate runners. Their explicit assignment lives in `.github/snapshot-shards.json`; `./snapshot-shards check` verifies every source suite appears exactly once. Use `./snapshot-shards balance --local` for a fresh local measurement or `./snapshot-shards balance --ci --runs 10` for medians from successful `main` runs, inspect the dry-run proposal, then repeat with `--write` to accept it. Multiple snapshot simulators on one Mac still contend for the same render server, so do not run the shards concurrently locally.
...
snapshot-shards     Validate/rebalance the two snapshot CI shards
