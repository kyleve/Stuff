...
struct ThrowSessionLocationTests {
    ...
    func committedLocationSurvivesRetryInterruption(
        ...
        let persisted = await preferenceStore.persistedPreferences()
        #expect(session.confirmedLocation?.position.coordinate.latitude == 40)
        #expect(session.confirmedLocation?.position.coordinate.longitude == -73)
        #expect(session.projectionMode == .trueSky)
        #expect(session.intensityPercent == 70)
        #expect(
            session.projectionPlaylist.entry(for: .airAndSpace)?.dwellDuration.seconds == 180,
