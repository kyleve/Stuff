...
/// Owns the app's single `WhereModel` and the `Launcher` that drives launch,
/// wiring both up at process launch rather than from a SwiftUI view's `.task`.
///
...
/// runs, so building the launcher here (whose synchronous prelude installs the
/// `CLLocationManager`) lets CoreLocation deliver the pending event, while the
/// async launch steps continue background tracking off the main thread.
@MainActor
final class AppDelegate: NSObject, UIApplicationDelegate, UNUserNotificationCenterDelegate {
    ...
    func application(
        ...
        // event; the launcher then skips foreground-only steps (onboarding) and
        // the UI builds no view tree.
        let reason: LaunchReason = launchOptions?[.location] != nil
            ? .background(.location)
            : .userForeground
        ...
        // The prelude installs the CLLocationManager synchronously so a queued
        // location event isn't lost; the rest (store open, etc.) runs as async
        // steps off this synchronous launch path.
