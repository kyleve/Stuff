...
public final class WhereSession {
    ...
    ///
    /// This is the imperative equivalent of `WhereLaunch.run`'s coordinator
    /// work steps, kept for previews/tests that drive the coordinator directly
    /// without the launch task. Report/data-issue loading is *not* here — the
    ...
    /// in its own right (see `WhereLaunch.run`).
    ...
    /// `WhereLaunch.run`); also re-run on every store change via
    ...
    /// launch step (see `WhereLaunch.run`).
    ...
    /// on persist. A launch step (see `WhereLaunch.run`); also runs on
    /// every foreground.
    ...
    /// (see `WhereLaunch.run`); also runs on every foreground.
    ...
    /// `applyReminderConfiguration()`. A launch step (see `WhereLaunch.run`);
    ...
    /// `WhereLaunch.run`); also runs on every foreground.
    ...
    /// teardown (see `WhereLaunch.reset`); throws on persistence failure
