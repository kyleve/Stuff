...
/// An explicitly activated developer launcher and tool surface above the app.
///
...
///
/// Attached once at ``RootView`` in every build after activation.
struct DeveloperOverlay: View {
    ...
    private func toggleMenu() {
        ...
        withAnimation(motion) {
            if model.presentation.isMenuPresented {
                ...
                whereModel?.porthole.captureMenuOrigin()
                model.openMenu()
            }
        }
    }
    ...
    private func openDestination(_ destination: DeveloperDestination) {
        switch destination {
            case .tool(.porthole):
                closeMenu()
                Task { await whereModel?.porthole.presentCurrentScreen() }
            case let .tool(tool):
    ...
    extension DeveloperOverlay: SnapshotProviding {
        static var snapshots: [SnapshotCase] {
            ...
            // Keep the native switch inside the first capture tile so this focused
            // row also checks its material rendering independently of the full menu.
            whereSnapshot(
                name: "LogViewModeAX",
                configurations: SnapshotConfiguration.combinations(
                    devices: [.iPhone],
                    colorSchemes: [.light, .dark],
                    dynamicTypes: [.accessibility5],
                ),
            ) {
                DeveloperOverlayPreview(presentation: .menu, surface: .logViewMode)
            }
            whereSnapshot(
                name: "PortholeEnabled",
                configurations: SnapshotConfiguration.combinations(
                    devices: [DeveloperOverlayPreview.menuContentFrame],
                    colorSchemes: [.light, .dark],
                    dynamicTypes: [.large, .accessibility5],
                ),
            ) {
                DeveloperOverlayPreview(
                    presentation: .menu,
                    corner: .topLeading,
