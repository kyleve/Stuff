...
- **Scope-tiered models** — scene-scoped **`YearReportModel`** (the selected
  ...
  **`LocationDayCountPresentationModel`** (the last primary-card counts the
  ...
  visible, then advances every changed number in one animated beat, adding one
  light haptic when any count increased; decreases, first visits, and newly
  appearing cards stay silent. Each model keeps its behavior off the view; none
  reimplements Core rules.
