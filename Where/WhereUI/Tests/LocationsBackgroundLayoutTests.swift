...
struct LocationsBackgroundLayoutTests {
    ...
    @Test func alternateRowsUseAHalfCellOffset() throws {
        ...
        #expect(first.frame.width < 64)
    }

    @Test func smallSetsRepeatAndEmptyInputsProduceNoCells() {
        let size = CGSize(width: 390, height: 844)
        #expect(LocationsBackgroundLayout.cells(count: 1, in: size, preferredCellSize: 64)
            .count > 1)
        #expect(LocationsBackgroundLayout.cells(count: 0, in: size, preferredCellSize: 64).isEmpty)
        #expect(LocationsBackgroundLayout.cells(count: 3, in: .zero, preferredCellSize: 64)
            .isEmpty)
    }
}
