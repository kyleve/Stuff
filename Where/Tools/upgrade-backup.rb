...
#     absent, and sets `formatVersion` to 3 (the current version). v3 expands
#     the sample-source vocabulary with `photo`; older samples need no reshape.
#
...
def upgrade_manifest(manifest)
  ...
  # v3 adds `photo` as a valid sample-source case. Existing source values are
  # already valid in v3, so upgrading requires only the version declaration.
