...
## P1s (Should do)
- refactor: Canonicalize Periscope `LogEvent.externalID`s onto the `store://` URL scheme. Once both the Periscope migration (#94) and the `store://` identity work (#96, `StoreURL` / `WhereStoreURLCodable`) land, replace the ad-hoc bare-string `externalID`s (`"2026-06-05"`, sample UUIDs, `"us-CA"`, a bare year) with the object's canonical `store://…` identity, so inspect-by-object shares one namespaced, collision-free key across DB rows, backups, and logs. Periscope's `externalID` stays an opaque `String` — this is purely an app-side convention (`someID.storeURL.absoluteString`), no Periscope change. Seams to resolve: (a) move `StoreURL` down from WhereCore to a module **RegionKit** can see (it's Foundation-only; the #96 doc already anticipates this) so `RegionAttributorLog` can use `store://regions/…`; (b) define `store://` identities for the object families that don't have one yet (days, manual days, samples, evidence) — that identity belongs with those types, not the log events; (c) decide on synthetic identities for non-row correlations (`YearReportModelLog`/`LoggedDaysModelLog` key on a year/scope, e.g. `store://reports/year?year=2025`). First natural adopter: `ResolveModelLog.dismissFailed`, whose `issueID` becomes a `DataIssueID` (already `store://issues/…`) in #96. Deferred deliberately out of #94 to avoid coupling two open PRs; log-event `externalID` tests were skipped in #94 for the same reason (they'd be rewritten here).
