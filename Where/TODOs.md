...
- fix: `setPrimaryRegions(_:)` skips the post-change reconcile fan-out. Region-picking onboarding (#95) made appearance edits user-reachable, but `WhereServices.setPrimaryRegions` is a bare `store.perform` — no `widgets.publish()`, reminder reconcile, or summary reconcile. In-app styling updates via `store.changes()` → `WhereSession.seedRegionStyles()`, but the widget snapshot (and daily summary notification body) can stay stale until the next publish/throttle window. Route through `DayJournal.reconcileAfterDayChange()` or at minimum `widgets.publish()`; add a regression test in `WidgetDataReaderTests` or `PrimaryRegionStoreTests`.
...
## P1s (Should do)
- feat: Region-picking onboarding + per-region customization (#95) — onboarding gate, `PrimaryRegion`/`RegionAppearance` model, settings region editor, backup v2 `primaryRegions` round-trip, `RegionAppearanceCatalog` + `RegionStyleResolver` in WhereUI.
