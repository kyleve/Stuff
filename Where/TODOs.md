...
- fix: `manualDay.Add_*` references depend on the **real-world date**: the compact `DatePicker` reserves its capsule width from *today's* date in medium format and renders the selection medium only if it fits, else falls back to short numeric ("7/15/26"). Diagnosed empirically (iOS 26.2): with today = Jul 17, a pinned Jul 15 selection renders short (references, recorded Jul 16, show medium "Jul 15, 2026"), while Jan 1 / Jan 2 / Mar 1 / today itself render medium — the flip tracks whether the selection's medium string is narrower than today's. So the Add references fail deterministically on days whose medium-format string is narrower than "Jul 15, 2026" and pass on wider ones; neither format is stable across days. Durable fix: render a deterministic date-label stand-in under `\.isCapturingSnapshot` (the regionMap approach) instead of the live compact picker, then re-record the Add references once. Reproduced at the branch base commit — not introduced by the phase 7–11 runner changes.
