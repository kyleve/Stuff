...
- **Tuist app-extension target** ([`Project.swift`](../../Project.swift),
  ...
  **PeriscopeCore**, and **SFSafeSymbols**. Embedded by the **Where** app.
  Shares the `group.com.stuff.where` App Group entitlement. Logs via the
  `WhereLog` facade (typed `ShareExtensionLog` events). As a separate process its
...
- **If a share has no loadable bytes, still compose** a metadata-only note
  ...
  "nothing, and here's why" must not be flattened into the same silence as "nothing
