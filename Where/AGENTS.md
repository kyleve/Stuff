...
- **`WhereServices.recentActivity`** (the on-demand Foundation Models
  ...
  surfaces as a typed reason. Never use a silent empty summary.
...
- **Launch and reset steps declare a budget, not a `measure` call.** Every step
  ...
  spanned by declaring `budget`. `MeasuredStep` pointedly isn't itself
...
- **Spans emitted before a scope's durable store attaches are
  ...
  The `SpanEnded` lands in the store. Durations survive but the pair doesn't.
...
- **The logging system is injected, not global.** `WhereModel.logSystem` has no
  default. A test can't silently attach sinks to `Periscope.shared`. (The
...
- **Demo mode asks for no permission and presents a granted user.** The
  ...
  `authorized: true`. No surface nags about a permission the demo can't obtain.
