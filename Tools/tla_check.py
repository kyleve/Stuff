...
def _contained_file(root: Path, relative_path: str, field: str) -> Path:
    ...
    try:
        ...
        raise TlaCheckError(
            f"error: {field} {relative_path!r} escapes {root}"
        ) from error
    if path.is_absolute():
        raise TlaCheckError(f"error: {field} {relative_path!r} must be relative")
    ...
    *,
    ...
    for case in manifest.cases:
        config_path = _contained_file(
            spec_directory, case.config, f"case {case.name!r} config"
        )
