...
struct ProfileServiceTests {
    @Test func sOnlyRunBuildsTestsAndReportsTypedXCResultData() async throws {
        ...
        let status = try await service.run(
            ProfileRequest(build: false, tests: true, snapshots: false),
        )
        ...
        #expect(invocations[2].captureOutput == false)
        #expect(invocations[2].mergeStandardError)
    ...
    @Test func buildOnlyPrintsTheParsedBuildTimingReport() async throws {
        ...
        let status = try await service.run(ProfileRequest(build: true, tests: false))
