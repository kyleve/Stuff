...
Xcode- and device-heavy macOS commands live in the independent
[`Tools/Package.swift`](Tools/Package.swift) package. Root command paths are
compatibility shims into its `stuff` executable. Cross-platform Ruby and Python
tools and bootstrap shell remain outside that package. Test the package with
`swift test --package-path Tools`. Its local [`AGENTS.md`](Tools/AGENTS.md)
owns process, filesystem, and command-parity rules.
...
Use `--dry-run` to validate an add or removal without changing files.
...
describes the build and device plan without signing, querying, installing, or
launching.
...
team needed). Its `--dry-run` performs no build, process, replacement, or launch.
