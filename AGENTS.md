...
(under `Shared/Porthole/`) adds macOS-capable libraries plus a
`.commandLineTool` (`porthole`) and a Mac Catalyst app (`PortholeApp`,
productName `Porthole`) in [`Project.swift`](Project.swift).
