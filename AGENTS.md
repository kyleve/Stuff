...
`worktree`, `xcstrings`, `attribution`, and `codex-watchdog`. Each takes
`--help`. Use one of these scripts instead of
...
over a build setting Xcode did not export.
...
selecting an iOS bundle. Tool-only changes remain covered by the same entry
...
- Setup first runs `./worktree --check-main`. That refreshes `origin/main` and
...
- Cleanup uses `./simulator --delete`. That deletes only that checkout's
