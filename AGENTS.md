...
- **Patchlight Catalyst is a separate compile gate.** `Patchlight-Catalyst`
  builds the iPad/Catalyst app at destination
  `platform=macOS,variant=Mac Catalyst`; the normal iOS test scheme does not
  prove the Catalyst variant or its sandbox entitlement compiles.
