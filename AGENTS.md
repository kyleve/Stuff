...
- **Building and testing** — [Build system](#build-system),
  ...
  [Generating the Xcode project](#generating-the-xcode-project), and the
  [Linux/Cloud caveats](#cursor-cloud-specific-instructions).
...
- **`TODOs.md`** — the durable backlog for a feature or module group. Two exist:
  ...
  (exploratory), nest dependent tasks under their parent, and **never delete a
  completed item** — move it to the "Completed issues" section at the bottom.
  A new area's `TODOs.md` copies that header.
- **`FLAKY_TESTS.md`** — generated by `./flaky`. Never hand-edit it; re-run the
  script.
- **`MODULE_AUDIT.md`** — a dated, read-only audit across every module, useful
  for finding known gaps and their severity. Treat it as a **snapshot, not a
  contract**: nothing regenerates it, and parts already name modules that have
  since been replaced (`Shared/LogKit` and `Shared/LogViewerUI` are now
  Periscope). Verify a finding against current source before acting on it.
